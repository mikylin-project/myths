...
public abstract class AbstractBiMap<K,V> implements BiMap<K,V> {
    ...
    AbstractBiMap(int size) {
        this(new HashMap<>(size),new HashMap<>(size));
    }

    AbstractBiMap(Map<K,V> normalMap,Map<V,K> reverseMap) {
        ...
        if(!Objects.equals(reverseMap.getClass(),normalMap.getClass()))
            throw new RuntimeException("map class must be same class.");
        if(Objects.equals(reverseMap,normalMap))
            throw new RuntimeException("map class must not be one.");
    ...
    public BiMap<K,V> set(K key, V value) {
        put(key,value);
        return this;
    }
